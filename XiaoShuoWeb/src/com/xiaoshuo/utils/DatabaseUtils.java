...
/**
 * 调用Apache Commons DBUtil组件的数据库操作类 采用DRUID作为数据源，数据源封装在类中
 */
public class DatabaseUtils {
	...
	public int update(String sql, int a) {
		return update(sql, null);
	}
	...
	public <T> List<T> find(Class<T> entityClass, String sql, Object[] params) {
		...
		try {
			if (params == null) {
				list = (List<T>) queryRunner.query(sql, new BeanListHandler<Object>(entityClass));
			} else {
				list = (List<T>) queryRunner.query(sql, new BeanListHandler<Object>(entityClass), params);
			}
		} catch (SQLException e) {
			LOG.error("Error occured while attempting to query data", e);
		}
	...
	public <T> List<T> find(Class<T> entityClass, String sql, Object[] params, int page, int pageSize) {
		...
		try {
			if (params == null) {
				list = (List<T>) queryRunner.query(sql + pageSql, new BeanListHandler<Object>(entityClass));
			} else {
				list = (List<T>) queryRunner.query(sql + pageSql, new BeanListHandler<Object>(entityClass), params);
			}
		} catch (SQLException e) {
			LOG.error("Error occured while attempting to query data", e);
		}
	...
	public <T> PageResult findPageResult(Class<T> entityClass, String sql, Object[] params, int page, int pageSize) {
		...
		return new PageResult(count, list, newCurrentPage, pageSize);
	}
	...
	public <T> T findFirst(Class<T> entityClass, String sql, Object[] params) {
		...
		try {
			if (params == null) {
				object = queryRunner.query(sql, new BeanHandler<Object>(entityClass));
			} else {
				object = queryRunner.query(sql, new BeanHandler<Object>(entityClass), params);
			}
		} catch (SQLException e) {
			LOG.error("Error occured while attempting to query data", e);
		}
